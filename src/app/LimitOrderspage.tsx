...
function loadOrders(): LimitOrder[] {
  try {
    const filePath = path.join(process.cwd(), 'data', 'LimitOrders.json');
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        saveOrders(json.limitOrders);
        saveWatchlist(json.watchlist);
        saveTPList(json.tplist);
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 space-y-10">
      ...
      <div className="flex gap-4 mb-4">
        ...
        <label
          className="bg-violet-500 hover:bg-violet-600 px-4 py-2 rounded text-white font-semibold cursor-pointer"
        >
          Import Data
          <input
            type="file"
      ...
      <form onSubmit={handleSubmit} className="bg-zinc-900 rounded-lg p-4 flex flex-wrap gap-4 items-center">
        ...
        <button
          type="submit"
          className="bg-teal-600 hover:bg-teal-700 px-5 py-2 rounded text-white font-semibold"
        >
          ...
          <div className="overflow-x-auto rounded-lg border border-zinc-800">
            <table className="w-full text-left table-auto">
              ...
              <tbody>
                ...
                {buyOrders.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-4 text-zinc-400">
                      No buy limit orders
                    </td>
                  </tr>
                )}
