...
export default function TPListPage() {
  ...
  const addOrUpdate = (e: React.FormEvent) => {
    ...
    if (editId) {
      setTPList(
        tplist.map((item) =>
          item.id === editId ? { ...item, ticker, target: tprice } : item
        )
      );
      setEditId(null);
    } else {
      ...
      setTPList([
        { id: Math.random().toString(36).slice(2), ticker, target: tprice },
        ...tplist,
      ]);
    }
    ...
    <div className="max-w-md mx-auto py-10 space-y-8">
      ...
      >
        <input
          ...
          value={input.ticker}
          onChange={(e) =>
            setInput((i) => ({ ...i, ticker: e.target.value }))
          }
          required
        ...
        <input
          ...
          value={input.target}
          onChange={(e) =>
            setInput((i) => ({ ...i, target: e.target.value }))
          }
          required
        ...
        <button
          ...
          aria-label={editId ? "Update Entry" : "Add Entry"}
        >
          {editId ? "Update" : "Add"}
        </button>
        {editId && (
      ...
      <div className="rounded bg-zinc-900 p-4">
        <table className="w-full table-auto">
          ...
          <tbody>
            {tplist.length === 0 && (
              <tr>
                <td colSpan={3} className="text-zinc-400 py-4 text-center">
                  No entries yet.
                </td>
              </tr>
            )}
